...
- answerKeys: 5-8 个答案关键词(用于思考题的结构化评分,每个词2-6个字,如"趋近""无限接近""不等于")
...
输出 JSON 格式:
- q: 题目
- points: 3-5 个评分关键要点(每个要点一句话,描述应该答出什么)
- answerKeys: 5-8 个答案关键词(用于关键词匹配评分,每个词2-6个字,如"趋近""无限接近""不等于")
...
请评分并给出反馈。注意：评分由结构化关键词匹配确定，你的主要职责是提供高质量的反馈。
...
输出 JSON 格式:
- score: 0-100 的整数（你的评分，系统会与关键词匹配分数对比）
